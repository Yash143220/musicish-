...
module.exports = {
  ...
  category: 'Filters',
  aliases: ['vw'],
  description: 'Set VaporMusicish Filter.',
  args: false,
  usage: '',
  userPrams: [],
  ...
  execute: async (message, args, client, prefix) => {
    ...
    const but_2 = new MessageButton()
      .setCustomId('vaporMusicish_but_')
      .setLabel('ON')
      .setStyle('PRIMARY')
      .setDisabled(true);
    ...
    collector.on('end', async () => {
      if (!m) return;
      await m.edit({
        embeds: [embed1.setDescription(`Time is Out type again ${prefix}vaporMusicish`)],
        components: [
    ...
    collector.on('collect', async (b) => {
      ...
      if (b.customId === 'clear_but') {
        ...
        return await b.editReply({
          embeds: [embed1.setDescription(`${emojiequalizer} VaporMusicish Mode Is \`OFF\``)],
          components: [row2],
        });
      } else if (b.customId === 'vaporMusicish_but') {
        ...
        return await b.editReply({
          embeds: [embed1.setDescription(`${emojiequalizer} VaporMusicish Mode Is \`ON\``)],
          components: [row1],
        });
      }
    });
